...
def cargar_estilos_extra():
    st.markdown("""
        <style>
            ...
            /* Estilo para los textos del plan */
            ...
            /* --- ESTILO DEGRADADO PARA EL BOTÓN DE BORRAR (POPOVER) --- */
            section[data-testid="stSidebar"] div[data-testid="stPopover"] > button {
                /* TRUCO PARA BORDE DEGRADADO + BORDE REDONDO */
                background: 
                    linear-gradient(#262730, #262730) padding-box, /* Fondo oscuro interior */
                    linear-gradient(90deg, #FF5F1F, #FFAA00) border-box; /* Borde degradado */
                
                border: 2px solid transparent !important; /* Espacio transparente para ver el degradado */
                border-radius: 50% !important; /* Totalmente redondo */
                
                color: #FF5F1F !important; /* Icono naranja */
                font-size: 14px !important;
                padding: 0px !important;
                
                /* Tamaño fijo y delicado */
            ...
            section[data-testid="stSidebar"] div[data-testid="stPopover"] > button:hover {
                box-shadow: 0 0 10px rgba(255, 95, 31, 0.5); /* Resplandor naranja */
                transform: scale(1.05);
                color: #FFAA00 !important; /* Icono se vuelve amarillo */
            }
...
def mostrar_modal_precios():
    ...
    with c1:
        ...
        <div class="price-card">
            ...
            <ul class="p-feat"><li>IA Básica</li><li>Texto</li><li>Historial</li></ul>
        </div>""", unsafe_allow_html=True)
        st.button("Tu Plan Actual", disabled=True, use_container_width=True, key="btn_free")
    with c2:
        st.markdown("""
        <div class="price-card" style="border-color: #FF5F1F;">
            <div class="p-title">PRO</div>
            <div class="p-price">$9<small>/mo</small></div>
            <ul class="p-feat"><li>IA Avanzada</li><li>Imágenes</li><li>Velocidad</li></ul>
        </div>""", unsafe_allow_html=True)
        if st.button("🚀 Activar PRO", type="primary", use_container_width=True, key="btn_pro"): simular_pago("pro")
    with c3:
        st.markdown("""
        <div class="price-card" style="border-color: #FFD700;">
            <div class="p-title" style="color:#FFD700">BUSINESS</div>
            <div class="p-price" style="color:#FFD700">$49<small>/mo</small></div>
            <ul class="p-feat"><li>Modelo Ultra</li><li>Dashboard</li><li>Soporte VIP</li></ul>
        </div>""", unsafe_allow_html=True)
...
def render_sidebar():
    ...
    st.markdown("""
        <style>
            section[data-testid="stSidebar"] div.block-container { padding-top: 1rem !important; padding-bottom: 0rem !important; }
                    ...
                    with st.form("f_login"):
                        ...
                        if st.form_submit_button("Entrar", type="primary", use_container_width=True):
                            if db.login(u, p):
                                ...
                                st.error("Usuario no encontrado en la base de datos local. Por favor regístrate.")
        ...
        with st.sidebar:
            ...
            with c_title:
                ...
                <div style='line-height: 1.1; margin-top: 5px;'>
                    <span style='font-weight: 800; font-size: 1.2rem; background: linear-gradient(90deg, #FF5F1F, #FFAA00); -webkit-background-clip: text; -webkit-text-fill-color: transparent;'>
                        KORTEXA AI
                    </span>
                    <br>
                    <span style='font-size: 0.75rem; color: #888;'>Inteligencia Neuronal</span>
                </div>
                """, unsafe_allow_html=True)
            ...
            if plan_real != "enterprise":
                if st.button("💎 Mejorar a Pro", type="primary", use_container_width=True):
                    mostrar_modal_precios()
            ...
            # --- HISTORIAL (BOTÓN BORRAR DEGRADADO) ---
            ...
            with c_hist_del:
                # Icono de papelera con Borde Degradado
                with st.popover("🗑️", use_container_width=False, help="Gestionar historial"):
                    ...
                    if st.button("Eliminar Chat Actual", type="primary", use_container_width=True):
                        if st.session_state.chat_id:
                            ...
                            st.warning("No hay chat activo.")
                            
                    # Divisor mini
                    st.markdown("""
                        <hr style="height: 1px; border: none; margin: 5px 0 8px 0; 
                        background: linear-gradient(90deg, transparent, #FF5F1F, #FFAA00, transparent); opacity: 0.5;">
                    """, unsafe_allow_html=True)
            ...
            with st.container(height=200, border=False):
                ...
                for sid, dat in sesiones:
                    bg_button = "secondary" 
                    if sid == st.session_state.chat_id: 
                        label_btn = f"➤ {dat.get('titulo','Chat')[:18]}.."
                    else:
                        label_btn = f"{dat.get('titulo','Chat')[:18]}.."
...
def render_mini_header():
    st.caption("Kortexa AI System v2.0")